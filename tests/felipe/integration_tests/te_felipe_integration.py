...
def test_register_and_login(client):
    ...
    response = client.post(
        "/users/", json={"email": "felipe@test.com", "password": "senha123"}
    )
    ...
    response = client.post(
        "/users/login", data={"username": "felipe@test.com", "password": "senha123"}
    )
...
def test_create_task(client):
    ...
    response = client.post(
        "/tasks/",
        json={"title": "Tarefa de Integração", "description": "Teste de integração"},
...
def test_delete_task(client):
    ...
    response = client.post(
        "/tasks/",
        json={"title": "Tarefa para Deletar", "description": "Descrição"},
        headers={"Authorization": f"Bearer {token}"},
    )
...
def test_list_tasks_shared(client):
    ...
    response = client.post(
        "/users/", json={"email": "amigo@test.com", "password": "senha456"}
    )
    assert response.status_code == 201
    assert response.json() == {"msg": "Usuário registrado com sucesso"}
